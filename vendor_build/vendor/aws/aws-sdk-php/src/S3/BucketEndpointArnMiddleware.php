...
use Unity3_Vendor\Aws\Arn\ArnParser;
use Unity3_Vendor\Aws\Arn\ObjectLambdaAccessPointArn;
use Unity3_Vendor\Aws\Arn\Exception\InvalidArnException;
use Unity3_Vendor\Aws\Arn\AccessPointArn as BaseAccessPointArn;
use Unity3_Vendor\Aws\Arn\S3\OutpostsAccessPointArn;
use Unity3_Vendor\Aws\Arn\S3\OutpostsArnInterface;
...
use Unity3_Vendor\Aws\Exception\UnresolvedEndpointException;
use Unity3_Vendor\Aws\S3\Exception\S3Exception;
...
class BucketEndpointArnMiddleware
{
            ...
            if (!empty($input = $service['shapes'][$op['input']['shape']])) {
                ...
                if (!empty($arnableKey) && ArnParser::isArn($cmd[$arnableKey])) {
                    try {
                        ...
                        if ($this->config['use_arn_region']->isUseArnRegion() && !$this->isFipsPseudoRegion($this->region)) {
    ...
    private function validateArn($arn)
    {
        if ($arn instanceof AccessPointArnInterface) {
            ...
            if ($this->isGlobal($this->region) && $this->config['use_arn_region']->isUseArnRegion() == \false && $arn->getRegion() != $this->region) {
