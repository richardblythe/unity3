...
trait EndpointRegionHelperTrait
{
    ...
    private function validateFipsConfigurations(ArnInterface $arn)
    {
        ...
        if ($this->isFipsPseudoRegion($this->region) && $this->stripPseudoRegions($this->region) != $arn->getRegion()) {
            throw new InvalidRegionException('Fips is currently not supported with cross region' . ' requests. Please provide a non-fips region or supply a matching' . ' access point ARN region and client region.');
        }
    }
