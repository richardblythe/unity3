...
use Unity3_Vendor\Aws\Api\DocModel;
use Unity3_Vendor\Aws\Api\Service;
use Unity3_Vendor\Aws\AwsClient;
use Unity3_Vendor\Aws\CacheInterface;
use Unity3_Vendor\Aws\ClientResolver;
use Unity3_Vendor\Aws\Command;
use Unity3_Vendor\Aws\Exception\AwsException;
use Unity3_Vendor\Aws\HandlerList;
use Unity3_Vendor\Aws\InputValidationMiddleware;
use Unity3_Vendor\Aws\Middleware;
use Unity3_Vendor\Aws\Retry\QuotaManager;
use Unity3_Vendor\Aws\RetryMiddleware;
use Unity3_Vendor\Aws\ResultInterface;
use Unity3_Vendor\Aws\CommandInterface;
use Unity3_Vendor\Aws\RetryMiddlewareV2;
use Unity3_Vendor\Aws\S3\UseArnRegion\Configuration;
use Unity3_Vendor\Aws\S3\UseArnRegion\ConfigurationInterface;
use Unity3_Vendor\Aws\S3\UseArnRegion\ConfigurationProvider as UseArnRegionConfigurationProvider;
use Unity3_Vendor\Aws\S3\RegionalEndpoint\ConfigurationProvider;
use Unity3_Vendor\GuzzleHttp\Exception\RequestException;
use Unity3_Vendor\GuzzleHttp\Promise\Promise;
use Unity3_Vendor\GuzzleHttp\Promise\PromiseInterface;
...
class S3Client extends AwsClient implements S3ClientInterface
{
    ...
    public function __construct(array $args)
    {
        ...
        if ($this->getConfig('bucket_endpoint')) {
            ...
            $stack->appendBuild(S3EndpointMiddleware::wrap($this->getRegion(), $this->getConfig('endpoint_provider'), ['dual_stack' => $this->getConfig('use_dual_stack_endpoint'), 'accelerate' => $this->getConfig('use_accelerate_endpoint'), 'path_style' => $this->getConfig('use_path_style_endpoint')]), 's3.endpoint_middleware');
        }
    ...
    public function createPresignedRequest(CommandInterface $command, $expires, array $options = [])
    {
        ...
        return $signer->presign(\Unity3_Vendor\Aws\serialize($command), $this->getCredentials()->wait(), $expires, $options);
    }
    /**
    ...
    public function getObjectUrl($bucket, $key)
    {
        ...
        return (string) \Unity3_Vendor\Aws\serialize($command)->getUri();
    }
    /**
    ...
    {
        if ($value) {
            $config = \Unity3_Vendor\Aws\Retry\ConfigurationProvider::unwrap($value);
