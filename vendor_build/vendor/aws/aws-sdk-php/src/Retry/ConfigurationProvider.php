...
namespace Unity3_Vendor\Aws\Retry;

use Unity3_Vendor\Aws\AbstractConfigurationProvider;
use Unity3_Vendor\Aws\CacheInterface;
use Unity3_Vendor\Aws\ConfigurationProviderInterface;
use Unity3_Vendor\Aws\Retry\Exception\ConfigurationException;
...
class ConfigurationProvider extends AbstractConfigurationProvider implements ConfigurationProviderInterface
{
    ...
    public static function env()
    {
        return function () {
            ...
            if (!empty($mode)) {
                return Promise\promise_for(new Configuration($mode, $maxAttempts));
            }
    ...
    {
        return function () {
            return Promise\promise_for(new Configuration(self::DEFAULT_MODE, self::DEFAULT_MAX_ATTEMPTS));
        };
    }
    /**
    ...
    public static function ini($profile = null, $filename = null)
    {
        ...
        return function () use($profile, $filename) {
            ...
            $data = \Unity3_Vendor\Aws\parse_ini_file($filename, \true);
            if ($data === \false) {
            ...
            return Promise\promise_for(new Configuration($data[$profile][self::INI_MODE], $maxAttempts));
        };
    }
    /**
